    ...
    <div className="operationExecutionsBox box shadow-box">
      ...
      <div>
        ...
        {props.operation?.operationGroupList?.map((operationGroup, i) => {
          return operationGroup.operationList?.map((operation, j) => {
            return (
              <>
                <p className="operationExecutionsBox__gridItem1">
                  {j === 0 ? (i + 1).toString() : <Image src={InternalOperationIcon} alt="internal operation icon" />}
                </p>
                <p
                  className="operationExecutionsBox__gridItem2 operationExecutionsBox__link"
                  title={operation.from.address}
                >
                  <Link href={'/' + operation.from.address}>
                    {operation.from.name ?? operation.from.address.slice(0, 8) + '...'}
                  </Link>
                </p>
                <p
                  className="operationExecutionsBox__gridItem3 operationExecutionsBox__link"
                  title={operation.to.address}
                >
                  <Link href={'/' + operation.to.address}>
                    {operation.to.name ?? operation.to.address.slice(0, 8) + '...'}
                  </Link>
                </p>
                    ...
                    <>
                      ...
                      {operation.assets.length > 0
                        ? operation.assets.every(asset => asset?.asset.name === operation.assets[0].asset.name)
                          ? `Transfer ${formatNumber(
                              +operation.assets.reduce((total, asset) => {
                                return total + +asset.quantity / 10 ** +asset.asset.decimals
                              }, 0),
                              locale,
                            )} ${operation.assets[0].asset.assetType === 'nft' ? operation.assets[0].asset.name : operation.assets[0].asset.ticker}`
