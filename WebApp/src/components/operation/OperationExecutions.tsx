				...
				{props.operation?.operationGroupList?.map((operationGroup, i) => {
					return (
						operationGroup.operationList?.map((operation, j) => {
							return (
								<>
									<p className="operationExecutionsBox__gridItem1">{j === 0 ? i.toString() : ""}</p>
									...
									<p className="operationExecutionsBox__gridItem4">{(operation.entrypoint !== 'transfer' && operation.entrypoint !== null) ? `Call ${operation.entrypoint}` : `Transfer ${formatNumber(+operation.quantity / 10**+operation.asset.decimals, locale)} ${operation.asset.assetType === 'nft' ? operation.asset.name : operation.asset.ticker}`}</p>
								</>
							);
						})
					);
