...
const Transaction = (props: Props) => {
  ...
  useEffect(() => {
    props.operation?.operationGroupList?.map(operationGroup => {
      return operationGroup.operationList?.map(operation => {
        return operation.assets.map(assetTransfer => {
  ...
  return transfers.length > 0 ? (
    <div className="operationTransfersBox box shadow-box">
      <h2>Transfers</h2>
      {}
          ...
          <div key={i} className="operationTransfersBox__transfer">
            ...
            <div className="operationTransfersBox__transfer__assets">
              {transfer.assets.map((asset, j) => {
                if (asset.asset.id === 'tezos') {
                  return (
                    <div className="operationTransfersBox__transfer__assets__div" key={j}>
                      <p>
                        {formatNumber(asset.quantity / 10 ** asset.asset.decimals, locale)}{' '}
                        {asset.asset.assetType === 'nft' ? asset.asset.name : asset.asset.ticker}
                      </p>
                      {asset.asset.image ? (
