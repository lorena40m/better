...
import { AccountTokens, Coin, Holding } from '@/pages/api/account-tokens'
      ...
      >
        ...
        <div>
          <p>
            {rates &&
              formatPrice(
                (+props.infos.balance / 10 ** 6) * rates.cryptos.XTZ +
                  props.tokens.coins.reduce(
                    (total, coin) =>
                      total +
                      (props.coinsInfos?.find(coinInfos => coinInfos.tokenAddress === coin.Address)?.exchangeRate ??
                        0) *
          ...
          <div
            className={open ? 'treasuryBox__head__arrow treasuryBox__head__arrow__open' : 'treasuryBox__head__arrow'}
          >
            <span></span>
            <span></span>
          </div>
        ...
        <div className="treasuryBox__body__selector">
          <div className="treasuryBox__body__selector__both">
            <h3
              className={
                coinsSelected
                  ? 'treasuryBox__body__selector__both__selected'
                  : 'treasuryBox__body__selector__both__notselected'
              }
              onClick={() => {
        ...
        {coinsSelected ? (
          <div className="coinBox__coin-container">
            {[
              ...
              .concat(props.tokens.coins)
              .map((coin: Holding<Coin>) => {
                let coinValue
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <p>
                        {formatPrice((+coin.quantity / 10 ** coin.asset.decimals) * coinValue, locale, rates?.fiats)}
                      </p>
                    </div>
                  </div>
                )
