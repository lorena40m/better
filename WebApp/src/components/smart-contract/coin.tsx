...
import { fetchCoinGraph, fetchCoinHolders } from '@/utils/apiClient'
...
export function Coin(props: Props) {
  ...
  function createPath(data) {
    ...
    setLineValues([
      minValue,
      parseFloat((minValue + (maxValue - minValue) / 4).toPrecision(4)),
      parseFloat((minValue + (maxValue - minValue) / 2).toPrecision(4)),
      parseFloat((minValue + ((maxValue - minValue) / 4) * 3).toPrecision(4)),
      maxValue,
    ])
  ...
  return (
    <div className="contract-coin shadow-box">
      <div className="contract-coin__header">
        ...
        <div>
          <h1>
            {props.infos.tokens[0].metadata?.name ?? props.infos.metadata?.name ?? props.infos2.account.name ?? 'Token'}
          </h1>
      ...
      {coinInfos ? (
        <div className="contract-coin__stats">
          ...
          <div>
            ...
            <p>
              {formatPrice(
                coinInfos.price.value *
                  (Number(props.infos.tokens[0].supply) / 10 ** props.infos.tokens[0].metadata.decimals),
                locale,
                rates?.fiats,
              )}
            </p>
        ...
        <div className="contract-coin__main">
          <div>
            ...
            {topHolders.map((holder, i) => {
              return (
                <Link
                  href={'/' + holder.account.address}
                  title={holder.account.address}
                  key={i}
                  className="contract-coin__main__holder"
                >
          ...
          <div>
            ...
            >
              ...
              <text x="90" y="150" fill="#19283b" textAnchor="end" dominantBaseline="middle" fontSize={26}>
                {formatPrice(lineValues[3], locale, rates?.fiats)}
              </text>
              <text x="90" y="280" fill="#19283b" textAnchor="end" dominantBaseline="middle" fontSize={26}>
                {formatPrice(lineValues[2], locale, rates?.fiats)}
              </text>
              <text x="90" y="410" fill="#19283b" textAnchor="end" dominantBaseline="middle" fontSize={26}>
                {formatPrice(lineValues[1], locale, rates?.fiats)}
              </text>
              <text x="90" y="540" fill="#19283b" textAnchor="end" dominantBaseline="middle" fontSize={26}>
                {formatPrice(lineValues[0], locale, rates?.fiats)}
              </text>

              <line
                x1={100 + (900 / 100) * svgMouseX}
                y1="0"
                x2={100 + (900 / 100) * svgMouseX}
                y2="540"
                stroke="#19283bAA"
                stroke-width="4"
                stroke-dasharray="5"
              />
              <path d={path} stroke="#19283b" strokeWidth="2" fill="none" />

              {
                <>
                  ...
                  />
                  <text
                    x={170 + (900 / 100) * svgMouseX - (svgMouseX > 50 ? 220 : 0)}
                    y={getPriceBoxInfos().y + 20}
                    fill="#19283b"
                    textAnchor="right"
                    ...
                    fontWeight={700}
                  >
                    {formatPrice(getPriceBoxInfos().value, locale, rates?.fiats)}
                  </text>
                  <text
                    x={120 + (900 / 100) * svgMouseX - (svgMouseX > 50 ? 220 : 0)}
                    y={getPriceBoxInfos().y - 30}
                    fill="#19283b"
                    textAnchor="left"
                    dominantBaseline="middle"
                    fontSize={26}
                  >
                    {getPriceBoxInfos().date}
                  </text>
                </>
              }
            </svg>
