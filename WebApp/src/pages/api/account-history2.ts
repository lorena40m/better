...
import { solveAddressName } from '@/utils/solve';
...
type History = Array<OperationBatch>
type OperationBatch = Array<Operation>
type Operation = {
  id: string,
  ...
  date: DateString,
  functionName: string,
  counterparty: false | { // counterparty is false when operationType is 'transferGroup'
    account: 'user' | 'baker' | 'ghostContract' | 'delegator' | 'contract' | 'asset',
    address: string,
    name: string,
    image: UrlString,
  },
...
export default async function handler(
  ...
  try {
    ...
    */
    ...
    console.log('idsForSearchingRoot', idsForSearchingRoot)
    ...
    const history: History = batches.map(([hash, batch]) => batch.map(root => {
      ...
      return {
        ...
        counterparty: operationType !== 'transferGroup' && (root.SenderAddress !== address ? {
          accountType: root.SenderAccountType == 2 ? contractTypes[root.SenderAccountKind] : accountTypes[root.SenderAccountType],
          id: root.SenderAddress,
          name: solveAddressName(root.SenderDomains, root.SenderDomainData),
          // image: UrlString,
        } : {
          accountType: root.TargetAccountType == 2 ? contractTypes[root.TargetAccountKind] : accountTypes[root.TargetAccountType],
          id: root.TargetAddress,
          name: solveAddressName(root.TargetDomains, root.TargetDomainData),
          // image: UrlString,
        }),
        transferedAssets:
          (xtzAmount => xtzAmount ? [{
          ...
          }] : [])(sum(operationsByRoot[root.Id].map(o => o.Amount)))
          .concat(
            []
            // Array.from(groupBy(transfersByRoot[root.Id], t => t.AssetId))
            // .map(group => [sum(group.map(t => t.Amount)), group])
