...
import { TestCase, TestScript, expect } from './framework'
...
const formatToken_CASES = [
  ...
  { quantity: '12345', decimals: 6, locale: 'fr', expect: '0,0123' },
  { quantity: '1234', decimals: 6, locale: 'fr', expect: '0,00123' },
  ...
  { quantity: '-123', decimals: 6, locale: 'fr', expect: '-0,000123' },
  { quantity: '-12', decimals: 6, locale: 'fr', expect: '-0,000012' },
  { quantity: '-1', decimals: 6, locale: 'en', expect: '-0.000001' },
...
TestScript(async function () {
  await TestCase('formatPrice', async function () {
    for (const CASE of formatPrice_CASES) {
      const result = formatPrice(CASE.quantity, CASE.locale, { 'EUR/USD': 0.9 })
      expect(
        `formatPrice("${CASE.quantity}", "${CASE.locale}", { 'EUR/USD': 0.90 }) should return "${CASE.expect}" ; got: "${result}"`,
        result === CASE.expect,
      )
    }
  })
  await TestCase('formatToken', async function () {
    for (const CASE of formatToken_CASES) {
      ...
      expect(
        `formatToken("${CASE.quantity}", ${CASE.decimals}, "${CASE.locale}") should return "${CASE.expect}" ; got: "${result}"`,
        result === CASE.expect,
      )
    }
  })
